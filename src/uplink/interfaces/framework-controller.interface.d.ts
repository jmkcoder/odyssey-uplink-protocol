 ...
 */
export interface TypedController<TBindings extends Record<string, Binding<any>> = Record<string, Binding<any>>, TMethods extends Record<string, (...args: any[]) => any> = Record<string, (...args: any[]) => any>, TEvents extends Record<string, EventEmitter<any>> = Record<string, EventEmitter<any>>> extends Controller {
    bindings: TBindings;
    methods?: TMethods;
    events?: TEvents;
}
...
export type ControllerEventHandlers<T extends Controller> = {
    [K in keyof T['events'] as `on${Capitalize<string & K>}`]?: T['events'][K] extends EventEmitter<infer U> ? (data: U) => void : never;
};
//# sourceMappingURL=framework-controller.interface.d.ts.map